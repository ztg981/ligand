...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "goal-summary") {
      ...
      // Parse messy free-text gym notes into a structured workout plan. Unlike
      // the other actions this returns JSON (as text), which the client parses.
      const importInstruction = "You are a fitness parser. Convert messy gym notes into a structured workout. Respond with ONLY valid minified JSON, no markdown, no code fences, no commentary. Shape: {\"exercises\":[{\"name\":string,\"muscleGroup\":one of chest|back|shoulders|biceps|triceps|legs|core|cardio,\"type\":\"strength\"|\"cardio\",\"targetSets\":number,\"targetReps\":number|null,\"targetWeight\":number|null,\"targetMinutes\":number|null}]}. Infer sensible sets/reps when the note only says 'heavy' or 'some'. Use null for weight when unspecified. Keep exercise names canonical (e.g. 'Bench Press', 'Incline Dumbbell Press', 'Lat Pulldown'). Cardio exercises use type 'cardio' with targetMinutes. Never invent exercises the note doesn't imply.";
      systemInstruction = importInstruction;
